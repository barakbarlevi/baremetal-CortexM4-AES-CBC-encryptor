...
bool simple_timer_has_elapsed(simple_timer_t* timer) {
    ...
    if (has_elapsed) {
        if(timer->auto_reset) {
            // target_time amount of time has passed
            uint64_t drift = now - timer->target_time;             // drift = how far ahead is now compared to target_time
            timer->target_time = (now + timer->wait_time) - drift; // Accounting for the drift
        } else {
